...
const PACK_NAME = 'Hiyuki System'
const PACK_AUTHOR = 'Adrien'
    ...
    try {
      ...
      if (!hasMedia || !/image|video|webp/.test(mime)) {
        return m.reply(
            ...
            `✦ [ ERROR DE ENTRADA ]\n` +
            `  ⟡ Debe adjuntar o responder a una imagen o video.\n` +
      ...
      try {
        ...
        return m.reply(`✦ [ ERROR ] Fallo al descargar el flujo de datos.`)
      }

      const isVideo = /video/.test(mime) || msg.gifPlayback
      if (isVideo && msg.seconds > 10) {
        return m.reply(`✦ [ RESTRICCIÓN ] El video no debe exceder los 10 segundos.`)
      }
      ...
      const pack = `${PACK_NAME} | ${groupName}`
      const author = PACK_AUTHOR
...
async function addExif(webpBuffer, metadata) {
  ...
  const json = {
    "sticker-pack-id": "hiyuki-protocol-3.0",
    "sticker-pack-name": metadata.packname,
    "sticker-pack-publisher": metadata.author,
    emojis: metadata.categories || ["❄︎"]
  }
