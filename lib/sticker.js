...
export async function sticker(imgBuffer, url, packname = 'Hiyuki System', author = 'Adrien | XLR4', extra = {}) {
    ...
    const isWebP = isRiff && head.slice(16, 24) === '57455250'
    const isVid = !isWebP && (
        head.startsWith('00000020') ||
        head.startsWith('00000018') ||
        isRiff
    )

    const inputExt = isVid ? '.mp4' : isWebP ? '.webp' : '.jpg'
    ...
    try {
        if (isVid) {
            await execAsync(
                ...
                `-vf "format=rgba,scale=512:512:force_original_aspect_ratio=decrease,` +
                `pad=512:512:(ow-iw)/2:(oh-ih)/2:color=0x00000000,fps=15" ` +
                `-loop 0 -preset default -an -vsync 0 -t 8 ` +
                `-vcodec libwebp_anim -lossless 0 -quality 80 "${outputPath}"`
            )
        } else {
            await execAsync(
                `ffmpeg -y -i "${inputPath}" ` +
