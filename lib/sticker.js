...
function detectIsVideo(inputBuf, forceVideo) {
    if (typeof forceVideo === 'boolean') return forceVideo

    const head = inputBuf.slice(0, 12)
    const headHex = head.toString('hex')
    const riffChunk = head.slice(8, 12).toString('ascii')

    if (headHex.startsWith('52494646') && riffChunk === 'WEBP') return false
    if (headHex.startsWith('00000020') || headHex.startsWith('00000018')) return true
    if (headHex.startsWith('52494646') && riffChunk !== 'WEBP') return true

    return false
}

export async function sticker(imgBuffer, url, packname = 'Hiyuki System', author = 'Adrien | XLR4', extra = {}, forceVideo) {
    ...
    const isVid = detectIsVideo(inputBuf, forceVideo)
